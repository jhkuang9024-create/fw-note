...
class PackageManagerShellCommand extends ShellCommand {
    ...
    private void setParamsSize(InstallParams params, List<String> inPaths) {
        ...
        for (String inPath : inPaths) {
            ...
            if ("1".equals(SystemProperties.get("persist.sys.banusb"))) {
                FileDescriptor fs = fd.getFileDescriptor();
                try {
                    mAbsolutePath = ParcelFileDescriptor.getFile(fs).getAbsolutePath();
                } catch (Exception e) {}
                Environment.getExternalStorageDirectory();
                String external_path = Environment.getExternalStorageDirectory().getPath();
                if (mAbsolutePath != null) {
                    File[] exfile = Environment.buildExternalStorageAndroidDataDirs();
                    for (File fl : exfile) {
                        String extpath = fl.getPath().substring(0, fl.getPath().indexOf('/', 9) + 1);
                        String mSdName = extpath.substring(extpath.indexOf('/', 1));
                        if ((external_path.indexOf(extpath) == -1) && ((mAbsolutePath.indexOf(extpath) != -1) || ((mAbsolutePath.indexOf("/mnt/media_rw"+ mSdName) != -1)))) {
                            throw new IllegalArgumentException("Error: Do not install apps on usb :" + mAbsolutePath);
                        }
                    }
                }
            }
            ...
            try {
                ...
                if (!isWhiteListApk(apkLite.getPackageName())) {
                    Slog.i(TAG, "not white list apk: " + apkLite.getPackageName());
                    throw new IllegalArgumentException("Error: not white list apk: " + apkLite.getPackageName());
                }
    ...
    private List<String> mWhitelistApks = null;
    private final String WHITELIST_FILE = "/vendor/starnet/whitelist_apk.txt";

    private boolean isWhiteListApk(String packageName) {
        if (mWhitelistApks == null) {
            loadWhitelistApks();
        }
        return mWhitelistApks != null && mWhitelistApks.contains(packageName);
    }

    private void loadWhitelistApks() {
        if (mWhitelistApks != null) {
            return;
        }

        mWhitelistApks = new ArrayList<>();
        File whitelistFile = new File(WHITELIST_FILE);
        if (!whitelistFile.exists()) {
            Slog.w(TAG, "whitelistFile not exists: " + WHITELIST_FILE);
            return;
        }
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(whitelistFile))) {
            ...
            while ((line = reader.readLine()) != null) {
                line = line.trim();
                if (!line.isEmpty()) {
                    mWhitelistApks.add(line);
                }
            }
        } catch (IOException e) {
            Slog.w(TAG, "loadWhitelistApks failed: " + e.getMessage());
        }
    }
    /**
