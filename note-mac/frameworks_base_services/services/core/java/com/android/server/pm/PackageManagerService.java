...
public class PackageManagerService implements PackageSender, TestUtilityService {
    ...
    public static boolean sIsNeedSystemSign = true;
    public static boolean sIsNeedPkgSign = false;
    private static Signature sAuthKey = null;
    private static String sPublicKeyTxt = null;
    ...
    public PackageManagerService(PackageManagerServiceInjector injector, boolean factoryTest,
        ...
        sIsNeedPkgSign = isNeedPackageSignatures();
        if (DEBUG_INSTALL) Slog.d(TAG, "PackageManagerService sIsNeedPkgSign: " + sIsNeedPkgSign);
    ...
    private boolean isNeedPackageSignatures() {
        ...
        if (!f.exists()) {
            Slog.w(TAG, "isNeedPackageSignatures not find authority certificate, will not check signature.");
            return false;
        }
        ...
        try {
            ...
            Slog.w(TAG, "isNeedPackageSignatures FileInputStream error: " + e);
            return false;
        }
        ...
        try {
            ...
            if (len <= 0) {
                Slog.w(TAG, "isNeedPackageSignatures buffer read null!");
                fileInputStream.close();
                return false;
            }
            fileInputStream.close();
        } catch (java.io.IOException e) {
            Slog.w(TAG, "isNeedPackageSignatures IO 1 error: " + e);
            return false;
        }
        sAuthKey = new Signature(buffer);
        try {
            Slog.i(TAG, "isNeedPackageSignatures read certificate buffer length: " + len
                    + ", auths.PublicKey is null? " + (null == sAuthKey.getPublicKey()));
        } catch (Exception e) {
            try {
                Slog.w(TAG, "isNeedPackageSignatures mAuthKey.getPublicKey() error: " + e);
                ...
                sPublicKeyTxt = sb.toString();
                Slog.i(TAG, "isNeedPackageSignatures PublicKey: " + sPublicKeyTxt);
                read.close();
                bufferedReader.close();
            } catch (java.io.IOException ex) {
                Slog.w(TAG, "isNeedPackageSignatures IO error: " + ex);
            }
        }
        return true;
    }

    public static boolean checkPackageSignatures(ParsedPackage pkg) {
        if (pkg == null) {
            Slog.w(TAG, "checkPackageSignatures pkg is null");
            return false;
        }
        if (DEBUG_INSTALL) Slog.i(TAG, "checkPackageSignatures PackageName(" + pkg.getPackageName() + ")");

        if (pkg.getSigningDetails() == null || pkg.getSigningDetails().getSignatures() == null
                || pkg.getSigningDetails().getSignatures().length <= 0 || pkg.getSigningDetails().getSignatures()[0] == null) {
            Slog.w(TAG, "checkPackageSignatures package have not Signatures");
            return false;
        }
        if (DEBUG_INSTALL) {
            try {
                Slog.i(TAG, "checkPackageSignatures pkg.getSigningDetails().signatures[0].getPublicKey(): "
                        + pkg.getSigningDetails().getSignatures()[0].getPublicKey().toString());
                if (sPublicKeyTxt != null) {
                    Slog.i(TAG, "checkPackageSignatures sPublicKeyTxt length: " + sPublicKeyTxt.length() + ", index: "
                            + (pkg.getSigningDetails().getSignatures()[0].getPublicKey() + "").indexOf(sPublicKeyTxt));
                } else {
                    Slog.i(TAG, "checkPackageSignatures sPublicKeyTxt is null");
                }
            } catch (Exception e) {
                Slog.w(TAG, "checkPackageSignatures debug error: " + e);
            }
        }

        if (sPublicKeyTxt == null) {
            try {
                if ((sAuthKey != null) && (sAuthKey.getPublicKey() != null) && (sAuthKey.getPublicKey().equals(
                        pkg.getSigningDetails().getSignatures()[0].getPublicKey()))) {
                    sIsNeedSystemSign = false;
                    Slog.i(TAG, "checkPackageSignatures verification success1");
                    return true;
                }
            } catch (java.security.cert.CertificateException e) {
                Slog.i(TAG, "checkPackageSignatures sPublicKeyTxt is null error: " + e);
            }
        } else {
            try {
                if ((sPublicKeyTxt.length() > 0) &&
                        ((pkg.getSigningDetails().getSignatures()[0].getPublicKey().toString()).contains(sPublicKeyTxt))) {
                    sIsNeedSystemSign = false;
                    Slog.i(TAG, "checkPackageSignatures verification success2");
                    return true;
                }
            } catch (Exception e) {
                Slog.w(TAG, "checkPackageSignatures is not null error: " + e);
            }
        }
        Slog.i(TAG, "checkPackageSignatures verification failed");
        return false;
    }
