...
final class InstallPackageHelper {
    ...
    private void preparePackageLI(InstallRequest request) throws PrepareFailure {
        ...
        if (parsedPackage.isTestOnly() && SystemProperties.getBoolean("sys.disable.apktestOnly", false)) {
        ...
        // "sys.limit.install.system.app" prop is work when "sys.limit.install.app" set as true
        boolean limitInstallSystemApp = SystemProperties.getBoolean("sys.limit.install.system.app", false);
        boolean isSystemApp = false;
        if (!limitInstallSystemApp) {
            PackageSetting packageSetting = mPm.mSettings.getPackageLPr(pkgName);
            if (packageSetting != null) {
                isSystemApp = PackageManagerServiceUtils.isSystemApp(packageSetting) || PackageManagerServiceUtils.isUpdatedSystemApp(packageSetting);
                if (DEBUG_INSTALL) Slog.w(TAG, "isSystemApp: " + isSystemApp);
            } else {
                Slog.w(TAG, "preparePackageLI PackageSetting is null");
            }
        } else {
            Slog.w(TAG, "preparePackageLI limitInstallSystemApp");
        }
        if (!isSystemApp && SystemProperties.getBoolean("sys.limit.install.app", false) && mPm.sIsNeedPkgSign
                && !mPm.checkPackageSignatures(parsedPackage)) {
            request.setReturnCode(PackageManager.INSTALL_FAILED_VERIFICATION_FAILURE);
            if (DEBUG_INSTALL) Slog.w(TAG, "setReturnCode return for INSTALL_FAILED_VERIFICATION_FAILURE");
            throw new PrepareFailure(INSTALL_FAILED_SESSION_INVALID,
                    "Fail to install app as not system app, limitInstallApp, needPkgSign, check app signatures fail");
        }
        ...
        synchronized (mPm.mLock) {
            ...
            if ((installFlags & PackageManager.INSTALL_REPLACE_EXISTING) != 0) {
                ...
                if (replace) {
                    ...
                    Slog.w(TAG, "hwc=====: " + oldPackage.getPackageName());
                    // Prevent persistent apps from being updated
                    if (oldPackage.isPersistent() && !"com.cmcc.jarvis".equals(oldPackage.getPackageName())
                ...
                if (ksms.shouldCheckUpgradeKeySetLocked(signatureCheckPs, signatureCheckSus,
                    ...
                    try {
                        ...
                        final boolean compatMatch =
                                ...
                                isRollback,PackageManagerService.checkPackageSignatures(parsedPackage));
