...
class ActivityStarter {
    ...
    private static final String SECOND_DISPLAY_APPS_PATH =
            "/vendor/etc/second_display_apps.xml";

    private static ArraySet<String> sSecondDisplayApps;
    ...
    int execute() {
        try {
            ...
            synchronized (mService.mGlobalLock) {
                ...
                if ("1".equals(android.os.SystemProperties.get("persist.dualscreen.display.type", "0"))) {
                    // Check if specific app should be launched on a designated display
                    overrideDisplayIdFromConfig();
                }
    ...
    private static ArraySet<String> loadSecondDisplayApps() {
        ArraySet<String> result = new ArraySet<>();

        File file = new File(SECOND_DISPLAY_APPS_PATH);
        if (!file.exists()) {
            Slog.w(TAG, "second_display_apps.xml not found");
            return result;
        }
        ...
        try {
            ...
            Slog.e(TAG, "read second_display_apps.xml failed", e);
        } finally {
    ...
    private boolean isSecondDisplayApp(String pkg) {

        if (sSecondDisplayApps == null) {
            sSecondDisplayApps = loadSecondDisplayApps();
        }

        return sSecondDisplayApps.contains(pkg);
    }

    private int getExternalDisplayId() {
        final int displayCount = mRootWindowContainer.getChildCount();

        for (int i = 0; i < displayCount; i++) {
            DisplayContent dc = mRootWindowContainer.getChildAt(i);
            if (dc == null) continue;
            int displayId = dc.mDisplayId;
            if (displayId == DEFAULT_DISPLAY) continue;

            DisplayInfo info = dc.getDisplayInfo();
            if (info == null) continue;
            if (info.type == Display.TYPE_EXTERNAL) {
                Slog.i(TAG, "getExternalDisplayId = " + displayId);
                return displayId;
            }
        }

        Slog.w(TAG, "No external display found, fallback to DEFAULT_DISPLAY");
        return DEFAULT_DISPLAY;
    }

    /**
     * Check /vendor/etc/second_display_apps.xml for app-to-display mapping and override the displayId
     * in ActivityOptions if a match is found.
     *
     * xml format:
     * <apps>
     ...
     *
     */
    private void overrideDisplayIdFromConfig() {
        if (mRequest.activityInfo == null) {
            return;
        }

        final String targetPackage = mRequest.activityInfo.packageName;
        if (targetPackage == null) {
            return;
        }
        int targetDisplayId = isSecondDisplayApp(targetPackage) ? getExternalDisplayId() : PRIMARY_DISPLAY_ID;
        if (mRootWindowContainer.getDisplayContent(targetDisplayId) == null) {
            Slog.w(TAG, "Display " + targetDisplayId + " not exist");
            return;
        }
        ...
        try {
            ...
            if (mRootWindowContainer.getDisplayContent(targetDisplayId) == null) {
                Slog.w(TAG, "Target display " + targetDisplayId
                        + " does not exist for package " + targetPackage);
                return;
            }

            // Update or create ActivityOptions with the target displayId
            if (mRequest.activityOptions != null) {
                ActivityOptions options = mRequest.activityOptions.getOptions(mRequest.intent,
                        mRequest.activityInfo, mService.getProcessController(mRequest.caller), mSupervisor);
                if (options != null) {
                    options.setLaunchDisplayId(targetDisplayId);
                    mRequest.activityOptions = SafeActivityOptions.fromBundle(
                            options.toBundle());
                } else {
                    ActivityOptions newOptions = ActivityOptions.makeBasic();
                    newOptions.setLaunchDisplayId(targetDisplayId);
                    mRequest.activityOptions = SafeActivityOptions.fromBundle(
                            newOptions.toBundle());
                }
            } else {
            ...
            Slog.i(TAG, "Overriding displayId to " + targetDisplayId
                    + " for package " + targetPackage + " from Config");
            return;
        } catch (NumberFormatException e) {
            Slog.w(TAG, "Invalid displayId format in app_display_map for "
                    + targetPackage + ": " + targetDisplayId);
        }
    }
    ...
    private int executeRequest(Request request) {
        ...
        if (aInfo != null) {
            boolean autoGrantAll = SystemProperties.getBoolean("persist.sys.pm.autogrant", true);
            if (autoGrantAll) {
                ...
                Slog.w(TAG, "requires permissions for package " + aInfo.packageName);
            }
        }
