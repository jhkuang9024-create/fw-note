    ...
    void dispatchProcessesChanged() {
        ...
        while (i > 0) {
            ...
            if (observer != null) {
                ...
                final int uid = mService.getObserverCallingUid(observer);
                boolean skipForegroundServices = false;
                if (uid != -1) {
                    try {
                        String[] packages = AppGlobals.getPackageManager().getPackagesForUid(uid);
                        if (packages != null) {
                            if (SystemProperties.get("sys.proj.type").equals("mobile")) {
                                for (String pkg : packages) {
                                    if ("com.android.systemui".equals(pkg)) {
                                        skipForegroundServices = true;
                                        break;
                                    }
                                }
                            }
                        ...
                        Slog.w(TAG_PROCESS_OBSERVERS, "Failed to get packages for UID " + uid, e);
                    }
                }
                ...
                try {
                    for (int j = 0; j < numOfChanges; j++) {
                        ...
                        if ((item.changes & ProcessChangeItem.CHANGE_FOREGROUND_SERVICES) != 0) {
                            ...
                            if (skipForegroundServices) {
                                if (DEBUG_PROCESS_OBSERVERS) {
                                    Slog.i(TAG_PROCESS_OBSERVERS,
                                    "Skipped onForegroundServicesChanged for SystemUI (uid=" + uid + ")");
                                }
                                continue;
                            }
