                ...
                try {
                    ...
                    final boolean compatMatch =
                            PackageManagerServiceUtils.verifySignatures(signatureCheckPs,
                                    ...
                                    compareRecover, isRollback,PackageManagerService.checkPackageSignatures(parsedPackage));
                    ...
                    if (sharedUserSetting != null) {
                        if (sharedUserSetting.signaturesChanged != null
                            ...
                            if (SystemProperties.getInt("ro.product.first_api_level", 0) <= 29) {
                                ...
                                final String pkgName = parsedPackage.getPackageName();
                                boolean isSystemApp = (scanFlags & PackageManagerService.SCAN_AS_SYSTEM) != 0;

                                if ("android.uid.system".equals(sharedUserSetting.name)
                                    && isSystemApp) {
                                    Slog.w("PackageManager",
                                        "Bypass signature check for system shared user pkg: "
                                            + pkgName);
                                } else {
