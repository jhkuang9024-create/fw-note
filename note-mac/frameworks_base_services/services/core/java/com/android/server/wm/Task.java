...
class Task extends TaskFragment {
    ...
    public void onConfigurationChanged(Configuration newParentConfig) {
        ...
        try {
            if ((mRootProcess != null) && (mRootProcess.mInfo != null) ) {
                ApplicationInfo info = mRootProcess.mInfo;
                int diff = getConfiguration().diff(newParentConfig);
                final UserHandle userHandle = UserHandle.getUserHandleForUid(info.uid);
                if (mRootProcess != null && mRootProcess.hasThread() && ((diff &ActivityInfo.CONFIG_SCREEN_SIZE) != 0)
                    && !CompatChanges.isChangeEnabled(157629738L, getBasePackageName(), userHandle)) {
                    CompatibilityInfo ci = mAtmService.compatibilityInfoForPackageLocked(info);
                    mRootProcess.getThread().updatePackageCompatibilityInfo(getBasePackageName(), ci);
                }
            }
