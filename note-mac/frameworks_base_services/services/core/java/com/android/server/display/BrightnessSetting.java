...
public class BrightnessSetting {
    ...
    private int mUserSerial;
     ...
     private void connectToProxy() {
         try {
             ...
             Slog.e(TAG, "connectToProxy: system control service not found."
                 + " Did the service fail to start?", e);
         } catch (RemoteException e) {
             Slog.e(TAG, "connectToProxy: system control service not responding", e);
         }
    }
    ...
    final class DeathRecipient implements HwBinder.DeathRecipient {
         ...
         public void serviceDied(long cookie) {
             if (SYSTEM_CONTROL_FRAMEOWKR_DEATH_COOKIE == cookie) {
                 Slog.e(TAG, "system control service died cookie: " + cookie);
                 mSysCtrl = null;
             }
         }
    }

    private void setBootenv(String prop, String val) {
         if (null == mSysCtrl)
             connectToProxy();

         try {
             Slog.d(TAG, "setBootenv: " + val);
             mSysCtrl.setBootEnv(prop, val);
         } catch (RemoteException e) {
             Slog.e(TAG, "setBootenv:" + e);
         }
    }
    ...
    public void setBrightness(float brightness) {
        ...
        synchronized (mSyncRoot) {
            ...
            int brtInt = BrightnessSynchronizer.brightnessFloatToInt(mBrightness) & 0x000000ff;
            setBootenv("ubootenv.var.bl_level", Integer.toString(brtInt));
