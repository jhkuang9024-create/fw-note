...
public class OomAdjuster {
    ...
    private void parseCustomsizedAppOomAdj(String file) {
        try {
            ...
            while ((line=br.readLine()) != null) {
                ...
                arrs=line.split(",");
                if (arrs.length < 2 || arrs[1].trim().isEmpty()) {
                    Slog.w(TAG, "Invalid oom adj config line: " + line);
                    continue;
                }
    ...
    private boolean applyOomAdjLSP(ProcessRecord app, boolean doingAll, long now,
        ...
        int changes = 0;
        if (mVec != null) {
            for (AppOomAdj appOomAdj : mVec) {
                if (appOomAdj.m_sAppName.equals(app.processName)) {
                    app.mState.setCurAdj(appOomAdj.m_nOomAdj);
                    // Slog.d(TAG, "applyOomAdjLocked pkg:" + app.processName +
                    //         " curAdj:" + app.curAdj);
                    break;
                }
            }
