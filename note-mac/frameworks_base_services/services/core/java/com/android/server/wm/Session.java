...
class Session extends IWindowSession.Stub implements IBinder.DeathRecipient {
    ...
    public Session(WindowManagerService service, IWindowSessionCallback callback) {
        ...
        mCanAddInternalSystemWindow = service.mContext.checkCallingOrSelfPermission(
                INTERNAL_SYSTEM_WINDOW) == PERMISSION_GRANTED || isOemCompatApp();
    ...
    private boolean isOemCompatApp() {
        final String[] OEM_WHITELIST = {
            "com.android.systemui"
        };
        String pkg = mService.mContext.getPackageManager().getNameForUid(mUid);
        Slog.d(TAG_WM, "isOemCompatApp " + pkg);
        return Arrays.asList(OEM_WHITELIST).contains(pkg);
    }
