...
bool BootAnimation::bootVideo() {
    ...
    if (ui::ROTATION_90 == mPhysicalRotation || ui::ROTATION_270 == mPhysicalRotation) {
    ...
    bool bForceOsd = android::base::GetBoolProperty("persist.sys.bootvideo.forceOsd", false);
    SLOGD("bForceOsd:%d", bForceOsd);
    if (bForceOsd) {
        Parcel request, tmpReply;
        request.setDataPosition(0);
        tmpReply.setDataPosition(0);
        int32_t method = INVOKE_ID_GET_AM_FORCE_USE_OSD;
        String16 token("android.media.IMediaPlayer");
        request.writeInterfaceToken(token);
        request.writeInt32(method);
        request.writeInt32(1);
        request.setDataPosition(0);
        mMediaPlayer->invoke(request, &tmpReply);
    }
...
BootAnimation::BootAnimation(sp<Callbacks> callbacks)
    ...
    char physicalRotate[PROPERTY_VALUE_MAX];
    if (property_get("persist.vendor.hwc.primary", physicalRotate, "0") > 0) {
        mPhysicalRotation = (ui::Rotation)atoi(physicalRotate);
        int rotation = atoi(physicalRotate);
        switch (rotation) {
            case 3:
                mPhysicalRotation = ui::ROTATION_90;
                break;
            case 4:
                mPhysicalRotation = ui::ROTATION_180;
                break;
            case 7:
                mPhysicalRotation = ui::ROTATION_270;
                break;
            default:
                mPhysicalRotation = ui::ROTATION_0;
                break;
        }
    } else {
        ALOGD("BootAnimation get rotation property error\n");
    }
        ...
        do {
            ...
            for (size_t i = 0; i < static_cast<size_t>(numEvents); i++) {
                ...
                if (event.header.type == DisplayEventReceiver::DISPLAY_EVENT_HOTPLUG) {
                    ...
                    if (property_get("persist.sys.builtinrotation", rotate, "0") > 0)
                        rotation = (ui::Rotation)atoi(rotate);
                    else
                        ALOGD("BootAnimation get property error\n");
                    if (ui::ROTATION_90 == rotation || ui::ROTATION_270 == rotation) {
...
void BootAnimation::resizeSurface(int newWidth, int newHeight) {
    ...
    SLOGD("resizeSurface BLASTBufferQueue to %d %d.", mWidth, mHeight);
...
bool BootAnimation::playAnimation(const Animation& animation) {
    const size_t pcount = animation.parts.size();
    nsecs_t frameDuration = s2ns(1) / animation.fps / 4;
...
#endif
                checkExit();
                if (j == fcount - 1) { // last frame
                    do {
                        usleep(100000); // sleep 0.1s
                    } while (checkExit() == 0);
                    ALOGD("Exiting animation loop at frame %zu of part %zu", j, i);
                    break;
                }
            }
